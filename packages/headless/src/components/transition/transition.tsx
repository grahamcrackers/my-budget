...
export type TransitionChildProps<TTag extends ReactTag> = Props<
    ...
    never,
    PropsForFeatures<typeof TransitionChildRenderFeatures> & TransitionClasses & TransitionEvents & { appear?: boolean }
>;
...
interface NestingContextValues {
    ...
    chains: MutableRefObject<Record<TransitionDirection, [container: ContainerElement, promise: Promise<void>][]>>;
...
function hasChildren(bag: NestingContextValues["children"] | { children: NestingContextValues["children"] }): boolean {
    if ("children" in bag) return hasChildren(bag.children);
    return (
        bag.current.filter(({ el }) => el.current !== null).filter(({ state }) => state === TreeStates.Visible).length >
        0
    );
}
...
function useNesting(done?: () => void, parent?: NestingContextValues) {
    ...
    const chains = useRef<Record<TransitionDirection, [identifier: ContainerElement, promise: Promise<void>][]>>({
        enter: [],
        leave: [],
        idle: [],
    });
    ...
    const onStart = useEvent(
        (container: ContainerElement, direction: TransitionDirection, cb: (direction: TransitionDirection) => void) => {
...
function TransitionChildFn<TTag extends ElementType = typeof DEFAULT_TRANSITION_CHILD_TAG>(
    ...
    if (appear && show && env.isServer) {
        theirProps = {
            ...
            // @ts-expect-error className seems to error out
    ...
    return (
        <>
            {!hasTransitionContext && hasOpenClosedContext ? (
                // maybe? @ts-expect-error This is an object
