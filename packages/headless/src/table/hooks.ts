        ...
        switch (action.type) {
                ...
                const columnCopy = state.columns.map((column) => {
                    ...
                    if (action.columnName === column.name) {
                        ...
                        const columnCompareFn =
                            column.sort || byTextAscending((object) => object.original[action.columnName]);
            ...
            case "SEARCH_STRING":
                ...
                stateCopySearch.rows = stateCopySearch.originalRows.filter(
                    (row) => row.cells.filter((cell) => cell.value.includes(action.searchString)).length > 0,
                );
...
const sortByColumn = <T extends DataType>(
    ...
    columns.forEach((column) => {
        ...
        if (sortColumn === column.name) {
            ...
            // default to sort by string
            const columnCompareFn = column.sort || byTextAscending((object) => object.original[sortColumn]);
